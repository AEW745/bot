...
module.exports = {
    name: 'Groupban',
    description: 'Ban a member from the Roblox Group.',
    data: new SlashCommandBuilder()
    .setName('groupban')
    .setDescription('Ban a member from the Roblox Group.')
    .addStringOption(option =>
        option.setName('username')
        .setDescription('User to Ban from Group').setRequired(true)
        .setAutocomplete(true)
        )
    .addStringOption(option =>
        option.setName('reason')
        .setDescription('Reason for Banning from Group')
        ),
        /**
         * 
         * @param {Client} bot
        ...
        async slashexecute(bot, interaction) {
            ...
            try {
                ...
                const groupbanchannel = await bot.guilds.cache.get(interaction.guild.id).channels.cache.get(serverlogs);
                let id;
                let rank;
                let role;
                try {
      ...
      let avatarurl = avatar[0].imageUrl;
            ...
            setTimeout(() => {
                ...
                groupbanchannel.send({ embeds: [embed] })
        } else {
                interaction.editReply({ content: `Failed to Ban **${username}** from the Roblox Group!\n**This message will Auto-Delete in 10 seconds!**`,
        }).then(
        setTimeout(() => {
            interaction.deleteReply().catch(() => {
                return;
