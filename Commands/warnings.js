          ...
          //let serversetup = await db.get(`ServerSetup_${interaction.guild.id}`)
            ...
            try {
                if (username) {
                  ...
                  if (!interaction.guild.members.me.permissions.any([PermissionsBitField.Flags.KickMembers])) return interaction.editReply(`:x:**ERROR** | I don't have permission to execute this command!\n**This message will Auto-Delete in 10 seconds!**`).then(
                  setTimeout(() => {
                    interaction.deleteReply().catch(() => {
                      return;
                    })
